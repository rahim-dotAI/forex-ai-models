"""
Trade Beacon v2.0.4 - Forex Signal Generator (PERFORMANCE-OPTIMIZED - FIXED)
==========================================

CRITICAL FIXES APPLIED:
- ✅ Fixed performance tracker compatibility (safe history access)
- ✅ Fixed expectancy field (supports both expectancy_per_trade and expectancy)
- ✅ Fixed global config updates after optimization
- ✅ Fixed missing reason initialization
...
# ✅ FIX 1: Validate mode AFTER log is created
...
def _default_config():
    return {
        ...
        "settings": {
            "aggressive": {
                "threshold": 42, "min_adx": 15, "rsi_oversold": 35,
                "rsi_overbought": 65, "min_volume_ratio": 1.0,
                "volume_penalty": 10, "min_risk_reward": 1.5,
                "atr_stop_multiplier": 2.5, "atr_target_multiplier": 5.0,
                "max_correlated_signals": 2
            },
            "conservative": {
                "threshold": 55, "min_adx": 22, "rsi_oversold": 30,
                "rsi_overbought": 70, "min_volume_ratio": 1.2,
                "volume_penalty": 15, "min_risk_reward": 2.0,
                "atr_stop_multiplier": 3.0, "atr_target_multiplier": 6.0,
...
# =========================
# ✅ FIXED: PERFORMANCE-BASED THRESHOLD OPTIMIZATION
# =========================
def optimize_thresholds_if_needed(config: Dict) -> Dict:
    ...
    FIXES APPLIED:
    - Safe history access with None check
    - Support both expectancy_per_trade and expectancy fields
    - Fixed reason initialization
    """
    ...
    # ✅ FIX 1: Safe performance tracker check
    if not PERFORMANCE_TRACKER or not getattr(PERFORMANCE_TRACKER, "history", None):
        log.info("⚙️ Performance tracker not available or empty, skipping optimization")
        return config
    ...
    # ✅ FIX 4: Safe float conversion for win_rate
    win_rate = float(stats.get("win_rate", 0)) / 100
    
    # ✅ FIX 2: Support both expectancy field names
    ...
    adjustment = 0
    reason = "Optimal"  # ✅ FIX 3: Initialize reason
...
def calculate_eligible_modes(score: int, adx: float, volume_ratio: float, 
    ...
    if (score >= conservative_settings["threshold"] and
        ...
        volume_ratio >= conservative_settings["min_volume_ratio"]):
    ...
    if (score >= aggressive_settings["threshold"] and
        ...
        volume_ratio >= aggressive_settings["min_volume_ratio"]):
...
# =========================
# SENTIMENT (Placeholder - Full implementation omitted for brevity)
# =========================
...
def write_dashboard_state(signals: list, successful_downloads: int, newsapi_calls: int = 0, marketaux_calls: int = 0,
    ...
    dashboard_data = {
        ...
        "stats": {
            ...
            # ✅ FIX 3: Support both expectancy field names in dashboard
            "expectancy": stats.get("expectancy_per_trade", stats.get("expectancy", 0))
        },
        "risk_management": {
